...
- (NSData *)encode:(short *)pcmBuffer length:(int)lengthOfShorts {
    ...
    decodedData = [[NSMutableData alloc] init];
	...
	int nSamples = (int)ceil(lengthOfShorts / encodeFrameSize);
...
- (NSData *)decode:(char *)encodedBytes length:(int)lengthOfBytes {
    ...
    int a = speex_decode_int(decodeState, &decodeSpeexBits, decodedBuffer);
    
    NSLog(@"decode = %d", a);
    
    NSData *pcmRawData =  [NSData dataWithBytes:decodedBuffer length: sizeof(short) * decodeFrameSize];
